        ...
        <div className='bg-pw h-screen w-full flex justify-center'>
            <div className="login__container border-2 border-tg h-1/3 w-1/3 bg-cs rounded-xl shadow-sm align-center">
                ...
                <div className="login__form">
                    <form className="flex flex-col items-center" onSubmit={(e) => {e.preventDefault();console.log(username)} }>
                        <div className="login__form__email w-3/4 mt-4">
                            <label className="text-tg text-sm font-bold" htmlFor="username">Username</label>
                            <input className="w-full h-8 border-2 border-tg rounded-md"  name="username" id="username"  value={username} onChange={(e) => setUsername(e.target.value)} />
                        </div>
